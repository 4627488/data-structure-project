...
本题目的核心逻辑集中在对 _父子关系_ 的维护（`children`）以及对 _成员详细信息_ 的维护（`members`）上，再配合一些辅助函数（如递归显示、查找、修改等），就形成了完整的家谱管理功能。
...
![显示整个家谱结构，用字符画表示](image-1.png)

![显示第 n 代所有人信息，以第二代为例](image-2.png)

![按姓名查询成员信息，查询“王文方”](image-3.png)

![按出生日期查询成员名单，查询“1974-11-12”](image-4.png)
...
函数中的JSON读取功能实则是复用 _选做题18 树的应用_ 中的 JSON 解析器，用于解析家谱数据。具体的 JSON 解析过程不在此赘述。
