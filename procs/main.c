...
void readProcs()
{
    ...
    {
        if (entry->d_type == DT_DIR && atoi(entry->d_name) > 0) // Check if the entry is a directory and the name is a number
        {
            char statPath[512];
...
void printFinProcs()
{
    ...
    while (cur)
    {
        printf("PID: %d, Name: %s, Duration: %d seconds, End Time: %s",
               cur->pid, cur->name, cur->dur, ctime(&cur->end));
        cur = cur->next;
    }
}
